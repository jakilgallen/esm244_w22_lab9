---
title: 'Time series continued, wrangling & visualizing spatial data'
...
- Any cyclicality evident? Any other notable patterns, outliers, etc.?
...
**Our takeaway here is similar:** 
- there is clear seasonality (higher values in winter months), with an increasingly evident second peak in June/July/August. 
- This reinforces our takeaways from the raw data and seasonplots. 

#### Decomposition (here by STL)

See Rob Hyndman's section on [STL decomposition](https://otexts.com/fpp2/stl.html) to learn how it compares to classical decomposition we did last week: 
...
To create the model below, we specify the model type (exponential smoothing, ETS), then tell it what type of seasonality it should assume using the `season("")` expression, where 
- "N" = non-seasonal (try changing it to this to see how unimpressive the forecast becomes!), 
- "A" = additive, 
- M" = multiplicative. 
